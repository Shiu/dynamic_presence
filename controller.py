...
class DynamicPresenceController(DataUpdateCoordinator):
    ...
    def __init__(self, hass: HomeAssistant, config_entry: ConfigEntry):
        ...
        self.room_name = config_entry.data.get(CONF_ROOM_NAME, "Unknown Room")
    ...
    def _load_config_values(self) -> None:
        ...
        self.presence_timeout = options.get(
            CONF_PRESENCE_TIMEOUT, NUMBER_CONFIG[CONF_PRESENCE_TIMEOUT]["default"]
        )
        ...
        self.active_room_timeout = options.get(
            CONF_ACTIVE_ROOM_TIMEOUT, NUMBER_CONFIG[CONF_ACTIVE_ROOM_TIMEOUT]["default"]
        )
        self.night_mode_timeout = options.get(
            CONF_NIGHT_MODE_TIMEOUT, NUMBER_CONFIG[CONF_NIGHT_MODE_TIMEOUT]["default"]
        )
        self.night_mode_start = options.get(
        ...
        self.night_mode_controlled_entities = options.get(
            CONF_NIGHT_MODE_CONTROLLED_ENTITIES, []
        )
        self.night_mode_entities_behavior = options.get(
            CONF_NIGHT_MODE_ENTITIES_BEHAVIOR, "additive"
        )
    ...
    async def async_added_to_hass(self) -> None:
        ...
        if state:
            ...
            _LOGGER.debug(
                "Restored state for %s: presence=%s, active_room=%s",
                self.room_name,
                self.presence_detected,
                self.is_active_room,
            )
        else:
            _LOGGER.debug("No state to restore for %s", self.room_name)
    ...
    def handle_presence_change(self, event: Event) -> None:
        ...
        if new_state is None:
            _LOGGER.warning(
                "Received presence change event with no new state for %s",
                self.room_name,
            )
            return
        ...
        _LOGGER.info(
            "Presence state changed to %s for %s",
            self.presence_detected,
            self.room_name,
        )
    ...
    def get_adjusted_timeout(self, timeout: int) -> int:
        if self.is_night_mode_active():
            ...
            _LOGGER.debug(
                "Adjusted timeout for %s: original=%s, night_mode_scale=%s, adjusted=%s",
                self.room_name,
                timeout,
                self.night_mode_scale,
                adjusted_timeout,
    ...
    def start_timer(self, timer_name: str) -> None:
        ...
        if timer_name == "presence":
            ...
            _LOGGER.error("Unknown timer: %s for %s", timer_name, self.room_name)
            return
        ...
        _LOGGER.debug(
            "Started %s timer for %s seconds in %s", timer_name, timeout, self.room_name
        )
    ...
    async def turn_on_controlled_entities(self):
        ...
        for entity_id in entities_to_turn_on:
            ...
            _LOGGER.info("Turning on %s with data: %s", entity_id, service_data)
    ...
    async def turn_off_controlled_entities(self):
        ...
        for entity_id in all_entities:
            domain = entity_id.split(".")[0]
            service = "turn_off"
            _LOGGER.info("Turning off %s", entity_id)
    ...
    def _validate_config(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if presence_timeout is not None:
            if not isinstance(presence_timeout, (int, float)) or presence_timeout <= 0:
                ...
                validated[CONF_PRESENCE_TIMEOUT] = NUMBER_CONFIG[CONF_PRESENCE_TIMEOUT][
                    "default"
                ]
            else:
        ...
        if active_room_threshold is not None:
            if (
                not isinstance(active_room_threshold, (int, float))
                ...
                validated[CONF_ACTIVE_ROOM_THRESHOLD] = NUMBER_CONFIG[
                    CONF_ACTIVE_ROOM_THRESHOLD
                ]["default"]
            else:
        ...
        if night_mode_scale is not None:
            if (
                not isinstance(night_mode_scale, (int, float))
                or night_mode_scale <= 0
                ...
                validated[CONF_NIGHT_MODE_SCALE] = NUMBER_CONFIG[CONF_NIGHT_MODE_SCALE][
                    "default"
                ]
            else:
