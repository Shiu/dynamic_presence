...
class DynamicPresenceSensor(DynamicPresenceEntity, SensorEntity):
    ...
    def __init__(self, coordinator, config_entry: ConfigEntry, description: SensorEntityDescription):
...
class PresenceDurationSensor(DynamicPresenceSensor):
    ...
    def native_value(self) -> int:
        ...
        if self.coordinator.presence_detected and self.coordinator.presence_start_time:
            return int((dt_util.utcnow() - self.coordinator.presence_start_time).total_seconds())
        return 0
...
class AbsenceDurationSensor(DynamicPresenceSensor):
    ...
    def native_value(self) -> int:
        ...
        """
        if not self.coordinator.presence_detected and self.coordinator.last_presence_end_time:
            return int((dt_util.utcnow() - self.coordinator.last_presence_end_time).total_seconds())
        return 0
...
class ActiveRoomStatusSensor(DynamicPresenceSensor):
    ...
    def native_value(self) -> str:
        ...
        """
        return "Active" if self.coordinator.is_active_room else "Inactive"
...
class PresenceSensorStateSensor(DynamicPresenceSensor):
    ...
    def native_value(self) -> str:
        ...
        """
        return "Detected" if self.coordinator.presence_detected else "Clear"
...
class NightModeStatusSensor(DynamicPresenceSensor):
    ...
    def native_value(self) -> str:
        ...
        return "Active" if self.coordinator.is_night_mode_active() else "Inactive"
