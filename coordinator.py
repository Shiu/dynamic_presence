...
class DynamicPresenceCoordinator(DataUpdateCoordinator):
    ...
    def _store_temp_manual_state(self, entity_id: str, state: bool):
        ...
        logCoordinator.debug(
            "Stored temporary manual state for %s: %s", entity_id, state
        )
    ...
    def _apply_temp_manual_states(self):
        ...
        if self._temp_manual_states:
            ...
            logCoordinator.debug(
                "Applied temporary manual states: %s", self._manual_states
            )
    ...
    def _update_manual_states(
        ...
        if self.data.get(f"{self.room_name}_occupancy_state") == "occupied":
                ...
                if new_state:
                    ...
                    if is_occupied:
                        ...
                        if self._are_all_lights_off():
                            ...
                            logCoordinator.debug(
                                "All lights off when leaving room, will clear states after grace"
                            )
                        else:
                            ...
                            if all_lights_on:
                                ...
                                logCoordinator.debug(
                                    "All lights on when leaving room, will clear states after timeout"
                                )
                            else:
